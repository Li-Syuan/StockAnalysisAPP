Arooncross <- function(
  ...
  add.indicator(strategy.st, name = "aroon", 
                arguments = list(HL = quote(mktdata[,c('high','close')]), n = n), 
                label="Aroon")
  relation <- c('gt','lt')
  ...
  add.signal(
    strategy.st, 
    name = "sigComparison",
    arguments = list(columns = c("aroonUp", "aroonDn"), relationship = relation[1]),
    label = "long.entry")
  add.signal(
    strategy.st, 
    name = "sigComparison",
    arguments = list(columns = c("aroonUp", "aroonDn"), relationship = relation[2]), 
    label = "long.exit")
  ...
  # -------------------------------------------------------------------------- #
  aron <- aroon(df[,c('high','close')],n = n)
